"""MyTool：用于查看和调整 AgentLoop 运行时状态的自省工具。

你可以把它理解成 Agent 的“自我查看面板”：

- ``check``：查看自己当前运行状态
- ``set``：在允许时修改部分运行时配置，或往 scratchpad 里记临时信息

它很强大，但也很敏感，所以文件里有大量保护规则。
"""
...
def _has_real_attr(obj: Any, key: str) -> bool:
    """判断对象是否真的有这个属性，而不是被 mock 动态伪造出来的。"""
...
class MyTool(Tool, ContextAware):
    ...
    def _format_value(val: Any, key: str = "") -> str:
        ...
        # 如果是 SubagentManager，就把展示逻辑委托到它的 _task_statuses 字典。
        ...
        # dict 小时直接展开，大时只展示 key，方便继续用点路径下钻。
